...
def split_location(x: str) -> list:
    ...
    res = [re.sub(r'[^a-zA-Z/ ]', '', i) for i in res]  # remove special characters
...
def process_text_data(ratings, users, books, tokenizer, model, vector_create=False):
    ...
    if vector_create:
        ...
        vector_lengths = [len(vec) for vec in book_summary_vector_list]
        ...
        np.save('./vector_data/text_vector/book_summary_vector.npy', book_summary_vector_list)        
        ...
        np.save('./vector_data/text_vector/user_summary_merge_vector.npy', user_summary_merge_vector_list)        
        
    else:
        print('Check Vectorizer')
        print('Vector Load')
        book_summary_vector_list = np.load('/data/ephemeral/home/data/text_vector/book_summary_vector.npy', allow_pickle=True)
        user_summary_merge_vector_list = np.load('/data/ephemeral/home/data/text_vector/user_summary_merge_vector.npy', allow_pickle=True)
...
def process_context_data(users, books):
    ...
    users_['age_range'] = pd.cut(users_['age'], 
                                bins=[0,10,20,30,40,50,60,70,max(users_.age)], 
                                labels=['0s','10s','20s','30s','40s','50s','60s','elder']).astype('category') 
...
def text_data_load(args):
    ...
    tokenizer = AutoTokenizer.from_pretrained(args.model_args[args.model].pretrained_model, token=True)
    model = AutoModel.from_pretrained(args.model_args[args.model].pretrained_model, token=True).to(device=args.device)
    model.eval()
    ...
    # age와 location(나라)의 cross product feature를 생성합니다.
    users_['age_country'] = (users_['location_country'].astype(str) + '_' + users_['age_range'].astype(str))
    user_features = ['user_id', 'age_range', 'location_country', 'location_state', 'location_city', 'age_country']
    book_features = ['isbn', 'summary', 'book_title', 'book_author', 'publisher', 'language', 'category', 'publication_range']
    ...
    delete_cols = ['summary']
    for df in [all_df, train_df, test_df]:
...
def text_data_loader(args, data):
    ...
    #                             )
    train_dataset = TensorDataset(torch.LongTensor(data['train'].values), torch.LongTensor(data['y_train'].values))
