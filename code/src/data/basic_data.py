...
def basic_data_setter(args, data):
    ...
    data['basic_train_dataset'], data['basic_valid_dataset'], data['basic_test_dataset'] = train_dataset, valid_dataset, test_dataset
    
    return data

def concat_data_loader(args, datas):
    train_dataset_list = []
    valid_dataset_list = []
    test_dataset_list = []
    
    for data in datas:
        for key, value in dict(list(data.items())[-3:]).items():
            if 'train' in key:
                train_dataset_list.append(value)
            elif 'valid' in key:
                valid_dataset_list.append(value)
            elif 'test' in key:
                test_dataset_list.append(value)

    combined_train_dataset = ConcatDataset(train_dataset_list)
    combined_valid_dataset = ConcatDataset(valid_dataset_list)
    combined_test_dataset = ConcatDataset(test_dataset_list)


    train_dataloader = DataLoader(combined_train_dataset, batch_size=args.dataloader.batch_size, shuffle=args.dataloader.shuffle, num_workers=args.dataloader.num_workers)
    valid_dataloader = DataLoader(combined_valid_dataset, batch_size=args.dataloader.batch_size, shuffle=False, num_workers=args.dataloader.num_workers) if args.dataset.valid_ratio != 0 else None
    test_dataloader = DataLoader(combined_test_dataset, batch_size=args.dataloader.batch_size, shuffle=False, num_workers=args.dataloader.num_workers)
